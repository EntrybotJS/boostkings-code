  ...
  socket.on('send message', function(data) {
    ...
    newChat.save(function(err, savedChat){
      if(err || !savedChat || !savedChat._id){
        ...
        Chat.findOne({_id: savedChat._id}).sort("created_on").populate('user').exec(function(err, message){
          if(err){
            console.log(err);
          }
          var outboundData = {}
...
app.use(
  ...
  function(req, res, next) {
    if (req.user && (req.user.group == "booster" || req.user.group == "admin")) {
      Order.find({isDone: false, requiresApproval: {$ne: true}}).populate('booster').exec(function(err, orders) {
        ...
        if(orders && orders.length > 0){
          ...
          orders.forEach(function(o){
            ...
            else if (o && o.booster && req.user && o.booster._id.toString() == req.user._id.toString()) boosterStats.todo += 1;
          })
