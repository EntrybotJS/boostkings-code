...
function getPrices(callback) {
  ...
  Price.getPrices(function(err, prices) {
     ...
     * Solo division boost
     *********************************************/
    pricesData.soloDivisionBoostTiers = ["Bronze", "Silver", "Gold", "Platinum", "Diamond", "Master"];
    ...
    for (i = 1; i < soloDivisionBoostDivs.length; i++) {
      soloDivisionBoostDivs[i].discountedPrice =
        soloDivisionBoostDivs[i].regularPrice * (1 - divisionBoostRebatePercentage);
    }
    // loop over every solo division to calculate the price
    // to reach that division
    for (i = 1; i < soloDivisionBoostDivs.length; i++) {
      soloDivisionBoostDivs[i].atomicPrice = soloDivisionBoostDivs[i].regularPrice;
      soloDivisionBoostDivs[i].regularPrice += soloDivisionBoostDivs[i - 1].regularPrice;
      soloDivisionBoostDivs[i].discountedPrice += soloDivisionBoostDivs[i - 1].discountedPrice;
    }
     ...
     * Duo division boost
     *********************************************/
    pricesData.duoDivisionBoostTiers = ["Bronze", "Silver", "Gold", "Platinum", "Diamond", "Master"];

    // factor in rebate percentage
    var duoDivisionBoostDivs = prices.duoDivBoost;
    for (i = 1; i < duoDivisionBoostDivs.length; i++) {
      duoDivisionBoostDivs[i].discountedPrice =
        duoDivisionBoostDivs[i].regularPrice * (1 - divisionBoostRebatePercentage);
    }

    // loop over every duo division to calculate the price
    // to reach that division
    for (i = 1; i < duoDivisionBoostDivs.length; i++) {
      duoDivisionBoostDivs[i].atomicPrice = duoDivisionBoostDivs[i].regularPrice;
      duoDivisionBoostDivs[i].regularPrice += duoDivisionBoostDivs[i - 1].regularPrice;
      duoDivisionBoostDivs[i].discountedPrice += duoDivisionBoostDivs[i - 1].discountedPrice;
    }
