  ...
  function(req, res, next) {
    Order.find({ isDone: false, booster: null })
      ...
      .exec(function(err, orders) {
        ...
        res.locals.pageDescription = "These are orders that do not have a booster assigned yet.";
  ...
  function(req, res, next) {
    Order.find({ isDone: false, booster: { $ne: null } })
      ...
      .exec(function(err, orders) {
        ...
        res.locals.pageDescription = "These are orders assigned to boosters but that are not completed yet.";
        res.locals.navId = "admin-orders";
        res.locals.orders = orders;
        return res.render("pages/admin/orders/list");
      });
  ...
  function(req, res, next) {
    if (req.body.action == "markAsDone") {
      Order.findOneAndUpdate(
        ...
        { $set: { requiresApproval: false, isDone: true, lolUsername: null, lolPassword: null } },
        { new: true }
      )
        .populate("user")
        .exec(function(err, updatedOrder) {
          ...
          var subject = "Boost Kings - Boost completed (order #" + updatedOrder.confirmationNumber.toUpperCase() + ")";
            ...
            html: pug.renderFile("views/emails/template.pug", {
              ...
              paragraphs: [
                ...
                "<b>Category</b>: " +
                  ...
                  "<br>",
                "<b>Server</b>: " + updatedOrder.getServer().name + "<br>",
          ...
          smtpTransport.sendMail(mailOptions, function(err) {
            req.flash("success", "Successfully marked as done. A confirmation email was sent to the customer.");
            return res.redirect(req.get("referer"));
          });
        });
  ...
  function(req, res, next) {
    Order.find({ requiresApproval: true })
      ...
      .exec(function(err, orders) {
        ...
        res.locals.pageDescription = "These orders were completed by a booster and need your approval.";
        res.locals.navId = "admin-orders";
        res.locals.orders = orders;
        return res.render("pages/admin/orders/list");
      });
  ...
  function(req, res, next) {
    Order.find({ isInThePool: true })
      ...
      .exec(function(err, orders) {
        ...
        res.locals.pageDescription = "These orders are available to all boosters.";
        res.locals.navId = "admin-orders";
        res.locals.orders = orders;
        return res.render("pages/admin/orders/list");
      });
  ...
  function(req, res, next) {
    ...
    Order.find()
      ...
      .exec(function(err, orders) {
        ...
        res.locals.pageDescription = "These are all orders ever placed on Boost Kings since the beginning.";
        res.locals.navId = "admin-orders";
        res.locals.orders = orders;
        return res.render("pages/admin/orders/list");
      });
